...
echo "=== N_m3u8DL-RE Core iOS Library 构建（使用本地 .NET） ==="
echo ""
...
"$DOTNET_BIN" publish "$CORE_PROJECT" \
    -c Release \
    -f net9.0-ios \
    -r ios-arm64 \
    -p:PublishAot=true \
...
# 显示生成的文件
if [ -d "output/ios-library" ]; then
    echo "生成的文件："
    ls -lh output/ios-library/*.dylib 2>/dev/null || echo "  未找到 .dylib 文件"
    ls -lh output/ios-library/*.h 2>/dev/null || echo "  未找到 .h 文件"
    echo ""
fi

echo "注意："
echo "  - 这是简化版本，仅包含设备版本 (arm64)"
echo "  - 如需完整的 XCFramework (包含模拟器)，请修改脚本添加模拟器构建"
echo ""
echo "下一步："
echo "  查看集成指南: docs/iOS-Library-Integration.md"
echo ""
